...
Catfeat=zeros(2*length(filesCat)+2*length(filesDog),1);
Dogfeat=zeros(2*length(filesCat)+2*length(filesDog),1);
...
for i = 1:length(filesCat)
    ...
    Catfeat(i+length(filesCat))=best;
...
for i = 1:length(filesCat)
    ...
    Sum_b=J(49,49)-Sum_w;
    haar1=Sum_w-Sum_b;
    Catfeat(i+2*length(filesCat))=haar1;
    im = imresize(im,[227 227]);
    ...
    scores = squeeze(gather(res(end).x)) ; 
    [bestScore, best] = max(scores);
    Catfeat(i+3*length(filesCat))=best;
...
for i = 1:length(filesDog)
    ...
    scores = squeeze(gather(res(end).x)) ; 
    [bestScore, best] = max(scores);
    Dogfeat(i+length(filesCat))=best;
...
for i = 1:length(filesDog)
    ...
    Sum_b=J(49,49)-Sum_w;
    haar1=Sum_w-Sum_b;
    Dogfeat(i+2*length(filesCat))=haar1;
    im = imresize(im,[227 227]);
    ...
    scores = squeeze(gather(res(end).x)) ; 
    [bestScore, best] = max(scores);
    Dogfeat(i+3*length(filesCat))=best;
